...
//    @SuppressWarnings("UnstableApiUsage")
    public static void main(String[] args) throws IOException {
        // Get all classes that start with prefix
        final ClassLoader loader = Thread.currentThread().getContextClassLoader();
        List<Class<?>> testsClasses = new ArrayList<>();

        for (ClassPath.ClassInfo info : ClassPath.from(loader).getTopLevelClasses()) {
            if (info.getName().startsWith("test.") && !info.getName().equalsIgnoreCase("test.BaseTest") && !info.getName().equalsIgnoreCase("test.Man")){
        ...
        final int MAX_PARRALEL_SESSION = 4;
        List<String> testGroupNames = new ArrayList<>();
        for (int index = 0; index < MAX_PARRALEL_SESSION; index++) {
        ...
        //Add all testes into suite
        boolean isTestingOnSafari = browser.equals("safari");
        suite.setTests(allTests);
        suite.setParallel(XmlSuite.ParallelMode.TESTS);
        suite.setThreadCount(isTestingOnSafari ? 1 : MAX_PARRALEL_SESSION);

//        //Run a group of test
//        if (isTestingOnSafari){
//            suite.addExcludedGroup("smoke");
//        } else {
